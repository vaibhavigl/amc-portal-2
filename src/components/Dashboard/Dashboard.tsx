...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const response = await axios.get('http://localhost:3001/api/contracts/stats/dashboard');
        ...
        <div className="flex items-start space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600 leading-relaxed">
              ...
              records of all your equipment maintenance agreements.
            </p>
            {stats?.expiringContracts && stats.expiringContracts > 0 && (
              <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <p className="text-yellow-800 text-sm font-medium">
                  ⚠️ You have {stats.expiringContracts} contract(s) expiring soon. 
                  Consider reviewing them for renewal.
                </p>
              </div>
            )}
          </div>
